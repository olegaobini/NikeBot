...
import nikebot.__version__ as __version__
import nikebot.globals as nikebot_globals
...
def check_adb_connection(device_id, wait_for_device):
    ...
    while True:
        ...
        if devices_count == 0:
            print_timeless(
                COLOR_HEADER + "Couldn't find any ADB-device available, sleeping a bit and trying again..." + COLOR_ENDC)
            sleep(10)
            continue
        ...
        print_timeless(COLOR_HEADER + "Couldn't find ADB-device " + device_id +
                       " available, sleeping a bit and trying again..." + COLOR_ENDC)
        sleep(10)
        continue
...
def execute_command(cmd) -> Optional[str]:
    try:
        cmd_res = subprocess.run(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, encoding="utf8")
    except IndexError:
    ...
    try:
        ...
        try:
            os.makedirs(os.path.join(
                "crashes", directory_name), exist_ok=False)
        except OSError:
        ...
        try:
            device.screenshot(os.path.join(
        ...
        try:
            device.dump_hierarchy(os.path.join(
        ...
        shutil.make_archive(os.path.join("crashes", directory_name),
                            'zip', os.path.join("crashes", directory_name))
        shutil.rmtree(os.path.join("crashes", directory_name))

        if nikebot_globals.is_nikebot():
            print(COLOR_OKGREEN + "Crash saved as \"crashes/" +
                  directory_name + ".zip\"." + COLOR_ENDC)
            print(COLOR_OKGREEN +
                  "Please attach this file if you gonna report the crash at" + COLOR_ENDC)
            print(COLOR_OKGREEN +
                  "https://github.com/alexal1/Insomniac/issues\n" + COLOR_ENDC)
    except Exception as e:
        print(
            COLOR_FAIL + f"Could not save crash after an error. Crash-save-error: {str(e)}" + COLOR_ENDC)
...
def print_copyright():
    if nikebot_globals.is_nikebot():
        print_timeless("\nIf you like this bot, please " +
                       COLOR_BOLD + "give us a star" + COLOR_ENDC + ":")
        print_timeless(
...
def _print_with_time_decorator(standard_print, print_time, debug, ui_log):
    def wrapper(*args, **kwargs):
        if nikebot_globals.is_ui_process and not ui_log:
            return
        ...
        if print_time:
            ...
            print_log += re.sub(r"\[\d+m", '',
                                ("[" + time + "] " + str(*args, **kwargs) + "\n"))
...
def _get_value(count, name, default, max_count, is_float):
    ...
    if max_count is not None and value > max_count:
        print(COLOR_FAIL + name.format(max_count) +
              f". This is max value." + COLOR_ENDC)
        value = max_count
    return value
...
def describe_exception(ex, with_stacktrace=True, context=None):
    ...
    trace = ''.join(traceback.format_exception(etype=type(
        ex), value=ex, tb=ex.__traceback__)) if with_stacktrace else ''
...
def _get_logs_dir_name():
    if nikebot_globals.is_ui_process:
...
def _get_log_file_name(logs_directory_name):
    ...
    log_name = f"{nikebot_globals.executable_name}_log-{curr_time}{'-'+nikebot_globals.execution_id if nikebot_globals.execution_id != '' else ''}.log"
...
class Logger(object):
    ...
    def _init_log(self):
        if not self.is_log_initiated:
            self.log = AnsiToWin32(open(_get_log_file_name(
                _get_logs_dir_name()), "a", encoding="utf-8")).stream
