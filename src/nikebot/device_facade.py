...
from nikebot.sleeper import sleeper
...
class DeviceFacade:
    ...
    def __init__(self, is_old, device_id, app_id, typewriter):
        ...
        if is_old:
            try:
                import uiautomator
                self.deviceV1 = uiautomator.device if device_id is None else uiautomator.Device(
                    device_id)
            except ImportError:
                raise ImportError(
    ...
    def find(self, *args, **kwargs):
        if self.deviceV1 is not None:
            import uiautomator
            try:
                print(self.deviceV1(*args, **kwargs))
    ...
    def back(self) -> bool:
        ...
        while not succeed:
            ...
            if not succeed:
                print(
                    COLOR_OKGREEN + "Pressed back but nothing changed on the screen. Will try again." + COLOR_ENDC)
    ...
    def start_screen_record(self, fps=10):
        ...
        if self.deviceV1 is not None:
            print(
                COLOR_FAIL + "Screen record doesn't work when you use the --old flag" + COLOR_ENDC)
        else:
            if not os.path.exists(SCREEN_RECORDS_PATH):
                os.makedirs(SCREEN_RECORDS_PATH)
            mp4_files = [f for f in listdir(
                SCREEN_RECORDS_PATH) if f.endswith(".mp4")]
            if mp4_files:
                last_mp4 = mp4_files[-1]
                debug_number = "{0:0=4d}".format(int(last_mp4[-8:-4]) + 1)
            else:
                debug_number = "0000"
            output = os.path.join(SCREEN_RECORDS_PATH,
                                  f"debug_{debug_number}.mp4")
            try:
                self.deviceV2.screenrecord(output, fps)
            except ModuleNotFoundError:
                print(
                    COLOR_FAIL + "To use screen recording please install additional packages:" + COLOR_ENDC)
                print(COLOR_FAIL + COLOR_BOLD +
                      'pip3 install -U "uiautomator2[image]" -i https://pypi.doubanio.com/simple' + COLOR_ENDC)
                return
            print(COLOR_OKGREEN +
                  f'Started screen recording: it will be saved as "{output}".' + COLOR_ENDC)
    ...
    def stop_screen_record(self):
        ...
        if is_recorded:
            ...
            if mp4_files:
                ...
                print(
                    COLOR_OKGREEN + f'Screen recorder has been stopped successfully! Saved as "{path}".' + COLOR_ENDC)
    ...
    def close_keyboard(self):
        ...
        if self.is_keyboard_open():
            ...
            if self.is_keyboard_open():
                print(
                    COLOR_FAIL + "Keyboard is open and couldn't be closed for some reason" + COLOR_ENDC)
            else:
                ...
                while attempts < max_attempts:
                    ...
                    try:
                        ...
                        if text is None:
                            if attempts < max_attempts:
                                print(
                                    COLOR_REPORT + "Could not get text. Waiting 2 seconds and trying again..." + COLOR_ENDC)
