...
class Sleeper:
    ...
    def random_sleep(self, multiplier=1.0):
        delay = uniform(self.sleep_range_start,
                        self.sleep_range_end) * multiplier
    ...
    def _set_random_sleep_range(self, speed, s1, s2):
        ...
        # x is a value between [0, 1]
        x = (speed - s1) / (s2 - s1) if s2 != s1 else 1
    ...
    def update_random_sleep_range(self):
        try:
            ...
            print(
                COLOR_FAIL + "Got an error while trying to measure the internet speed" + COLOR_ENDC)
...
def _get_internet_speed():
    from nikebot.tools import speedtest
